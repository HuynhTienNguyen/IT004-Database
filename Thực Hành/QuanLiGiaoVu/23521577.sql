...
--CACH 2
CREATE TRIGGER MAHV ON HOCVIEN
FOR INSERT
AS BEGIN
	DECLARE @MAHV3L VARCHAR(3), @MAHV2R VARCHAR(2), @MALOP VARCHAR(3), @SISO INT
	SELECT @MAHV3L = LEFT(I.MAHV, 3), @MAHV2R = CAST(RIGHT(I.MAHV, 2) AS INT), @MALOP = I.MALOP, @SISO = L.SISO
	FROM INSERTED I
	JOIN LOP L ON I.MALOP = L.MALOP
	IF (@MAHV3L = @MALOP AND @MAHV2R >= 0 AND @MAHV2R <= @SISO) BEGIN
		PRINT('ERROR')
...
--I.23: Khi đăng kí môn học mới, phải xét đến thứ tự trước sau giữa các môn học (sau khi học
